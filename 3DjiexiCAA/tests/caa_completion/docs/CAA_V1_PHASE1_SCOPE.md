...
- Implemented Hole, Pad, Pocket, constant Edge Fillet, Chamfer, Shell, Thickness, Shaft, Groove, Rib, Slot and Boolean operation payload extraction through R21 Public CAA interfaces, reported separately from type recognition as `native_feature_parameter_extraction`.
...
- Feature families with missing field-level contract coverage or incomplete formal V5R21 sample coverage remain outside the accepted claim until real samples are parsed and validated: Draft, Rectangular/Circular/User Pattern, variable Fillet, face Fillet and tritangent Fillet. They must not be counted as Phase 1 complete from source code or payload row counts alone.
- Pattern remains outside the accepted claim even though the parser has compiled `CATIAPattern`/`CATIARectPattern`/`CATIACircPattern`/`CATIAUserPattern` branches: the current formal `PD-PATTERN-01` catalog expects rectangular, circular and user variants, but the available `pd_patterns.CATPart` real run currently exposes only a rectangular pattern row.
