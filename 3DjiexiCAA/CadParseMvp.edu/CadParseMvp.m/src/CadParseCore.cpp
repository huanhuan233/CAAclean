// ???????? API ????????????????????Decoder ???Generic/Opaque ????? JSON ??‰^
                                        ...
                                        const char* message, const std::string& feature_id)
{
  ...
  // setw(6) ?? setfill('0') ?????????????¦Ë??size()+1 ??????????? D000001 ?????
...
std::string FeatureTypeFingerprintBuilder::StableKey(const TypeFingerprint& fingerprint)
{
  ...
  // C++03 ??§Ù?¦¶ for????????? const_iterator ??????? vector??
...
// ???????? SHA-256 ????? 32 ¦Ë????????VS2008 Win32 ?? unsigned long ? 32 ¦Ë??
...
// ??????????? ASCII ?????? V5Rxx/SP/HF???§Õ source_file_hint ????e????????·Ú??
