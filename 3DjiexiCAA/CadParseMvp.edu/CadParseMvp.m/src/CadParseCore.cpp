...
// 用途：确保每个 Hole 候选恰好落入一个终态，防止接口异常被静默漏计。
...
// 用途：把通用 Registry 的布尔 Match 契约桥接到显式候选状态。
...
namespace
{
// 用途：以 C++03 可用方式拒绝 NaN 和无穷值，防止无效坐标进入 JSON。
...
// 用途：通过 API 无关 Hole 视图确认专用接口并建立 Typed Hole 载荷；失败交回 Generic。
